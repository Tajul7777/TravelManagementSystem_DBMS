<?php
session_start();
$conn = mysqli_connect("localhost", "root", "", "travel");

if ($conn->connect_errno > 0) {
    die("Unable to connect: " . $conn->connect_error);
}

//fetch user info using session
$user_email = $_SESSION['user_email'];
$user_password = $_SESSION['user_password'];

//fetch user info using email and password
$query_user = "SELECT * from users where user_email='" . $user_email . "' and user_password='" . $user_password . "' ";
$result_user = mysqli_query($conn, $query_user);
$row_user = mysqli_fetch_row($result_user);

$user_id = $row_user[0];
$user_firstName = $row_user[1];
$user_lastName = $row_user[2];
$phn_num = $row_user[7];

//fetch pack_id using hyperlink in package.php
$pack_book_id = intval($_GET['pack_id']);

//fetch package info using pack_id
$query_pack = "SELECT * FROM packages where pack_id=" . $pack_book_id;
$result_pack = mysqli_query($conn, $query_pack);
$row_pack = mysqli_fetch_row($result_pack);

//join packages+pack_plan+stayplan to fetch inclusion table details
$query_inclusion = "SELECT stayplan.daynum, stayplan.placename, stayplan.sightseeing, stayplan.hotels, stayplan.meals, stayplan.transport
                            FROM (packages NATURAL JOIN pack_plan) NATURAL JOIN (stayplan)
                            WHERE pack_id=" . $pack_book_id;
$result_inclusion = mysqli_query($conn, $query_inclusion);

//fetch info for review details
if (isset($_POST['review'])) {
    $star = $_POST['star'];
    $message = $_POST['message'];

    //insert review id+rating+message into review table
    $sql_reviews = "INSERT INTO `review`(`review_rating`, `review_details`) VALUES ('" . $star . "','" . $message . "')";
    $results_review = mysqli_query($conn, $sql_reviews);

    //fetch the last inserted review details
    $query_review = "SELECT * FROM (
                    SELECT * FROM review ORDER BY review_id DESC LIMIT 1
                    ) as t ORDER BY review_id";
    $result_review = mysqli_query($conn, $query_review);
    $row_review = mysqli_fetch_row($result_review);
    $review_id = $row_review[0];

//    echo "".$review_id;
//    echo "".$star;
//    echo "".$message;
//    echo "".$user_id;
//    echo "".$pack_book_id;
//

    //insert details into review_details
    $sql_reviewdetails = "INSERT INTO `review_details`(`review_id`, `review_rating`, `review_details`, `user_id`, `pack_id`) VALUES"
        . " ('" . $review_id . "','" . $star . "','" . $message . "','" . $user_id . "','" . $pack_book_id . "')";
    $result_reviewdetails = mysqli_query($conn, $sql_reviewdetails);

    if ($result_reviewdetails == TRUE && $result_review == TRUE) {
        header("Location: http://localhost/project_dbms/packages.php");
    } else {
        echo 'Something wrong in review details or review';
    }
}
$review_slide = "SELECT review_id, review_rating, review_details, pack_id
                FROM review_details
                WHERE pack_id='" . $pack_book_id . "'
                ORDER BY review_id DESC LIMIT 3";
$result_reviewSlide = mysqli_query($conn, $review_slide);


?>
<!DOCTYPE html>
<!--
To change this license header, choose License Headers in Project Properties.
To change this template file, choose Tools | Templates
and open the template in the editor.
-->
<html>
<head>
    <meta charset="UTF-8">
    <title>Your Trip Our Plan</title>

    <meta name="format-detection" content="telephone=no"/>
    <link href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css" rel="stylesheet"
          integrity="sha384-BVYiiSIFeK1dGmJRAkycuHAHRg32OmUcww7on3RYdg4Va+PmSTsz/K68vbdEjh4u" crossorigin="anonymous">
    <link href="https://maxcdn.bootstrapcdn.com/font-awesome/4.7.0/css/font-awesome.min.css" rel="stylesheet"
          integrity="sha384-wvfXpqpZZVQGK6TAh5PVlGOfQNHSoD2xbE+QkPxCAFlNEevoEH3Sl0sibVcOQVnN" crossorigin="anonymous">
    <link rel='stylesheet' type='text/css' href="css/stylesheet.css"/>
    <link href='//fonts.googleapis.com/css?family=Open+Sans:400,700,600' rel='stylesheet' type='text/css'>
    <link href='//fonts.googleapis.com/css?family=Roboto+Condensed:400,700,300' rel='stylesheet' type='text/css'>
    <link href='//fonts.googleapis.com/css?family=Oswald' rel='stylesheet' type='text/css'>
    <link rel="stylesheet" href="css/font-awesome.css">
    <link rel="stylesheet" type="text/css" href="css/pack_details.css">
    <link rel="icon" href="images/favicon.ico">
    <link rel="shortcut icon" href="images/"/>
    <link rel="stylesheet" href="booking/css/booking.css">
    <link rel="stylesheet" href="css/camera.css">
    <link rel="stylesheet" href="css/owl.carousel.css">
    <link rel="stylesheet" href="css/packages.css">
    <script type="text/javascript" src="js/pack_details.js"></script>
    <script type="text/javascript" src="js/jquery.js"></script>
    <script src="js/jquery-migrate-1.2.1.js"></script>
    <script src="js/"></script>
    <script src="js/superfish.js"></script>
    <script src="js/jquery.ui.totop.js"></script>
    <script src="js/jquery.equalheights.js"></script>
    <script src="js/jquery.mobilemenu.js"></script>
    <script src="js/jquery.easing.1.3.js"></script>
    <script src="js/owl.carousel.js"></script>
    <script src="js/camera.js"></script>
    <script src="js/jquery.mobile.customized.min.js"></script>
    <script src="booking/js/booking.js"></script>
    <script src="js/home.js"></script>
    <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.2.1/jquery.min.js"></script>

    <style>

        * {
            margin: 0;
            padding: 0;
        }

        a {
            text-decoration: none !important;
        }
    </style>
    <style>   /*body { background: #000; }*/

        .collapse {
            /*  width: 93%;
              max-width: 350px;
              text-align: center;
              margin: 4% auto;
              padding: 30px 0;
              background: #111;
              color: #EEE;
              border-radius: 5px;
              border: thin solid #444;
              overflow: hidden;*/
            margin-left: -24px;
        }

        hr {
            margin: 20px;
            border: none;
            border-bottom: thin solid rgba(255, 255, 255, .1);
        }

        div.title {
            font-size: 2em;
        }

        h1 span {
            font-weight: 300;
            color: #Fd4;
        }

        div.stars {
            width: 270px;
            display: inline-block;
        }

        input.star {
            display: none;
        }

        label.star {
            float: right;
            padding: 10px;
            font-size: 36px;
            color: #444;
            transition: all .2s;
        }

        input.star:checked ~ label.star:before {
            content: '\f005';
            color: #FD4;
            transition: all .25s;
        }

        input.star-5:checked ~ label.star:before {
            color: #FE7;
            text-shadow: 0 0 20px #952;
        }

        input.star-1:checked ~ label.star:before {
            color: #F62;
        }

        label.star:hover {
            transform: rotate(-15deg) scale(1.3);
        }

        label.star:before {
            content: '\f006';
            font-family: FontAwesome;
        }
    </style>
    <!-- Review CSS-->
    <style>
        .carousel-control.left, .carousel-control.right {
            background: none;
            width: 25px;
        }

        .carousel-control.left {
            left: -25px;
        }

        .carousel-control.right {
            right: -25px;
        }

        .broun-block {
            background: url("http://myinstantcms.ru/images/bg-broun1.jpg") repeat scroll center top rgba(0, 0, 0, 0);
            padding-bottom: 34px;
        }

        .block-text {
            background-color: #fff;
            border-radius: 5px;
            box-shadow: 0 3px 0 #2c2222;
            color: #626262;
            font-size: 14px;
            margin-top: 27px;
            padding: 15px 18px;
        }

        .block-text a {
            color: #7d4702;
            font-size: 25px;
            font-weight: bold;
            line-height: 21px;
            text-decoration: none;
            text-shadow: 0 1px 2px rgba(0, 0, 0, 0.3);
        }

        .mark {
            padding: 12px 0;
            background: none;
        }

        .block-text p {
            color: #585858;
            font-family: Georgia;
            font-style: italic;
            line-height: 20px;
        }

        .sprite {
            background-image: url('data:image/png;base64,');
        }

        .sprite-i-triangle {
            background-position: 0 -1298px;
            height: 44px;
            width: 50px;
        }

        .block-text ins {
            bottom: -44px;
            left: 50%;
            margin-left: -60px;
        }

        .block {
            display: block;
        }

        .zmin {
            z-index: 1;
        }

        .ab {
            position: absolute;
        }

        .person-text {
            padding: 10px 0 0;
            text-align: center;
            z-index: 2;
        }

        .person-text a {
            color: #ffcc00;
            display: block;
            font-size: 14px;
            margin-top: 3px;
            text-decoration: underline;
        }

        .person-text i {
            color: #fff;
            font-family: Georgia;
            font-size: 13px;
        }

        .rel {
            position: relative;
        }
    </style>
    <!-- Slider-->
    <script>
        $(document).ready(function () {
            $(".btn1").click(function () {
                $(".stayplan").hide();
            });
            $(".btn2").click(function () {
                $(".stayplan").show(3000);

            });
        });</script>

</head>
<body>
<header>
    <div class="container_12">
        <div class="grid_12">
            <div class="menu_block">
                <nav class="horizontal-nav full-width horizontalNav-notprocessed" style="list-style: none;">
                    <ul class="sf-menu" style="list-style: none;">
                        <li><a href="home.php">ABOUT</a></li>
                        <li class="current"><a href="packages.php">HOT TOURS</a></li>
                        <li><a href="index-2.html">SPECIAL OFFERS</a></li>
                        <li><a href="index-3.html">BLOG</a></li>
                        <li><a href="index-4.html">CONTACTS</a></li>
                    </ul>
                </nav>
                <div class="clear"></div>
            </div>
        </div>
        <div class="grid_12">
            <h1>
                <a href=".home.php">
                    <img src="a_pics/logo.png" alt="Bangladesh Tourism" class="img-responsive">
                </a>
            </h1>
        </div>
    </div>
</header>

<!--      Header Part  Start  -->

<div class="selectroom">
    <div class="container">
        <div class="selectroom_top">
            <div class="col-md-4 selectroom_left" data-wow-delay="">
                <img src="a_pics/<?php echo "$row_pack[7]"; ?>" alt="">
            </div>
            <div class="col-md-8 selectroom_right" data-wow-delay="">
                <h2></h2>
                <p class="dow"
                   style="font-family: 'Amethysta', serif; font-size: 15px;"><?php echo "$row_pack[0]"; ?></p>
                <p class="fa fa-angle-double-down" style="font-family: 'Amethysta', serif; font-size: 15px;"><b>Package
                        Type :</b> <?php echo "$row_pack[2]"; ?></p><br>
                <p class="fa fa-location-arrow " style="font-family: 'Amethysta', serif; font-size: 15px;"><b>Package
                        Location :</b> <?php echo "$row_pack[3]"; ?></p><br>
                <p class="fa fa-square-o " style="font-family: 'Amethysta', serif; font-size: 15px;"><b>Features :</b>
                    Air Conditioning , Balcony / Terrace / Customizable Tour / Satellite </p><br>
                <p class="fa fa-dashboard " style="font-family: 'Amethysta', serif; font-size: 15px;"><b>Pay & Hold
                        :</b>Pay Tk 1,00 per person now and hold the
                    package at this price, payment as per policy can be made in the next 24/48 hrs. Holding of seats are
                    subject to availability and in case of non availability of selected seats you can choose from a wide
                    range of departures. To avail this option, click "Book Now".</p>
                <p class="fa fa-calculator" style="font-family: 'Amethysta', serif; font-size: 15px;"><b>Strat From
                        :</b> <?php echo "$row_pack[4]"; ?></p>

                <br>
                <div class="col-sm-4" style="font-family: 'Amethysta', serif; font-size: 30px;">
                    <p><b class="headline" style="font-size: 30px;">Inclusions</b></p>
                    <ul style="list-style: none;">
                        <li><a class="fa fa-plane"></a></li>
                        <li style="margin-left: 2px;"><a class="fa fa-home"></a></li>
                        <li style="margin-left: 10px;"><a class="fa fa-bars"></a></li>
                        <li style="margin-left: 10px;"><a class="fa fa-cutlery"></a></li>
                    </ul>
                </div>
                <div class="container col-sm-4 review1">
                    <div class="btn_review">
                        <button class="headline btn btn-primary btn1" style="font-size: 20px; margin-left: 10px;"
                                data-toggle="collapse" data-target="#demo">Review
                        </button>
                    </div>
                    <div class="collapse" id="demo">
                        <div class="stars">
                            <form action="" method="post"><br>
                                <input class="star star-5" id="star-5" type="radio" name="star" value="5"/>
                                <label class="star star-5" for="star-5"></label>
                                <input class="star star-4" id="star-4" type="radio" name="star" value="4"/>
                                <label class="star star-4" for="star-4"></label>
                                <input class="star star-3" id="star-3" type="radio" name="star" value="3"/>
                                <label class="star star-3" for="star-3"></label>
                                <input class="star star-2" id="star-2" type="radio" name="star" value="2"/>
                                <label class="star star-2" for="star-2"></label>
                                <input class="star star-1" id="star-1" type="radio" name="star" value="1"/>
                                <label class="star star-1" for="star-1"></label>
                                <input class="form-control" name="message" style="width: 250px; height: 100px;;"
                                       placeHolder="Message"
                                       data-constraints='@NotEmpty @Required @Length(min=20,max=999999)'>
                                <br>
                                <label class=""></label><br>
                                <input class="btn btn2" name="review" type="submit" value="Submit"/>
                            </form>
                        </div>
                    </div>
                </div>
                <div class="col-sm-4 stayplan" style="font-family: 'Amethysta', serif; font-size: 30px;">
                    <p class=""><b class="headline" style="font-size: 30px;">Stay Plan </b></p>
                    <p class="">Srimangal 1 Night</p>
                    <p class="">Moulvibazar 2 Night</p>
                    <p class="">jaflong & Radargul 1 Night</p>
                    <p class="">Bachnakandi 1 Night</p>
                    </ul>
                </div>


            </div>
            <div class="clearfix"></div>
        </div>
    </div>
</div>
<!-- Header Part End -->
<!--     Package Details Start  -->
<div class="selectroom" style="font-family: 'Amethysta', serif; font-size: 20px; margin-top: -100px;">
    <div class="container">
        <div class="row">
            <div class="selectroom_top">
                <h3 style="color: white; text-align: center;" class="headline">Package Details</h3>
                <p style="padding-top: 1%;">&quot;
                    <?php echo "$row_pack[6]"; ?> </p>
            </div>
        </div>
    </div>
</div>
<!--     Package Details End  -->
<!-- Inclusion Table Start  -->
<div class="selectroom" style="margin-top: -100px;">
    <div class="container">
        <div class="row">
            <div class="selectroom_top">
                <h3 style="color: white; text-align: center;" class="headline"> InClusion Table</h3><br><br>
                <table class="table table-striped" style="font-family: 'Amethysta', serif; font-size: 20px;">
                    <thead>
                    <tr>
                        <th>Day</th>
                        <th>Place Name</th>
                        <th>Sight Seeing</th>
                        <th>Hotels</th>
                        <th>Meals</th>
                        <th>Transport</th>
                    </tr>
                    </thead>
                    <?php while ($rows = mysqli_fetch_row($result_inclusion)) {
                        ?>
                        <tbody>
                        <tr>
                            <th scope="row"><?php echo "$rows[0]"; ?></th>
                            <td><?php echo "$rows[1]"; ?></td>
                            <td><?php echo "$rows[2]"; ?></td>
                            <th scope="row"><?php echo "$rows[3]"; ?></th>
                            <td><?php echo "$rows[4]"; ?></td>
                            <td><?php echo "$rows[5]"; ?></td>
                        </tr>
                        </tbody>
                    <?php } ?>
                </table>
            </div>
        </div>
    </div>
</div>
<!-- Inclusion Table End  -->

<!-- Review Table Start -->
<div class="selectroom">
    <div class="container">
        <div class="row">

            <div class="selectroom_top">
                <h2 class="headline" style="font-size: 30px; text-align: center;">Recent Reviews</h2>
            </div>
        </div>
        <div class="carousel-reviews broun-block">
            <div class="container">
                <div class="row">
                    <div id="carousel-reviews" class="carousel slide" data-ride="carousel">

                        <div class="carousel-inner">

                            <?php while ($row_reviewSlide = mysqli_fetch_row($result_reviewSlide)) { ?>
                                <div class="col-md-4 col-sm-6">
                                    <div class="block-text rel zmin">
                                        <a title=""
                                           href="#"><?php echo "$user_firstName" . " " . "$user_lastName" ?></a>
                                        <div class="mark">My rating:
                                            <?php if ($row_reviewSlide[1] == 5) {
                                                echo '<span class="rating-input">
               <span  class="glyphicon glyphicon-star"></span> 
<span  class="glyphicon glyphicon-star"></span> 
<span  class="glyphicon glyphicon-star"></span> 
<span  class="glyphicon glyphicon-star"></span>
<span  class="glyphicon glyphicon-star"></span>

 </span>';

                                            } elseif ($row_reviewSlide[1] == 4) {
                                                echo '<span class="rating-input"><span  class="glyphicon glyphicon-star"></span><span
                                                         class="glyphicon glyphicon-star"></span><span
                                                         class="glyphicon glyphicon-star"></span><span
                                                        class="glyphicon glyphicon-star"></span><span
                                                        
                                                        class="glyphicon glyphicon-star-empty"></span></span>';


                                            } elseif ($row_reviewSlide[1] == 3) {
                                                echo '<span class="rating-input"><span  class="glyphicon glyphicon-star"></span><span
                                                         class="glyphicon glyphicon-star"></span><span
                                                         class="glyphicon glyphicon-star"></span><span
                                                        class="glyphicon glyphicon-star-empty"></span><span
                                                        
                                                        class="glyphicon glyphicon-star-empty"></span></span>';


                                            } elseif ($row_reviewSlide[1] == 2) {
                                                echo '<span class="rating-input"><span  class="glyphicon glyphicon-star"></span><span
                                                         class="glyphicon glyphicon-star"></span><span
                                                         class="glyphicon glyphicon-star-empty"></span><span
                                                        class="glyphicon glyphicon-star-empty"></span><span
                                                        
                                                        class="glyphicon glyphicon-star-empty"></span></span>';


                                            } elseif ($row_reviewSlide[1] == 1) {
                                                echo '<span class="rating-input"><span  class="glyphicon glyphicon-star"></span><span
                                                         class="glyphicon glyphicon-star-empty"></span><span
                                                         class="glyphicon glyphicon-star-empty"></span><span
                                                        class="glyphicon glyphicon-star-empty"></span><span
                                                        
                                                        class="glyphicon glyphicon-star-empty"></span></span>';


                                            } else {
                                                echo '<span class="rating-input"><span  class="glyphicon glyphicon-star-empty"></span><span
                                                         class="glyphicon glyphicon-star-empty"></span><span
                                                         class="glyphicon glyphicon-star-empty"></span><span
                                                        class="glyphicon glyphicon-star-empty"></span><span
                                                        
                                                        class="glyphicon glyphicon-star-empty"></span></span>';


                                            }


                                            ?>

                                        </div>
                                        <p><?php echo "$row_reviewSlide[2]" ?></p>
                                        <ins class="ab zmin sprite sprite-i-triangle block"></ins>
                                    </div>
                                    <div class="person-text rel">
                                        <img src="#"/>
                                        <a title="" href="#"></a>
                                        <i></i>
                                    </div>
                                </div>
                            <?php } ?>
                        </div>
                        <a class="left carousel-control" href="#carousel-reviews" role="button" data-slide="prev">
                            <span class="glyphicon glyphicon-chevron-left"></span>
                        </a>
                        <a class="right carousel-control" href="#carousel-reviews" role="button" data-slide="next">
                            <span class="glyphicon glyphicon-chevron-right"></span>
                        </a>

                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
<!-- Review Table End  -->
<!--     Booking  Start  -->
<!--     Booking Button   -->
<div class="selectroom">
    <div class="container">
        <div class="row">
            <div class="selectroom_top">
                <ul style="list-style: none;">
                    <li align="center">
                        <button type="button" class="btn btn-primary btn-xl" name="submit"
                                style="color: white; font-family: 'Amethysta', serif; font-size: 15px;"><a
                                    href="book_info.php?pack_id=<?php echo $pack_book_id; ?>">Book</a></button>
                    </li>
                </ul>
            </div>
        </div>
    </div>
</div>
<!--     Booking Button End   -->
<script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/js/bootstrap.min.js"
        integrity="sha384-Tc5IQib027qvyjSMfHjOMaLkfuWVxZxUPnCJA7l2mCWNIpG9mGCD8wGNIcPD7Txa"
        crossorigin="anonymous"></script>
<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.2.1/jquery.min.js"></script>

</body>
</html>
